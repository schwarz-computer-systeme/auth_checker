...
my %log_files = (
        ...
        'postfix' => '/usr/local/bin/data/mail.info',
...
my $u1 = "\nauth_checker.pl STP/POP3 Authentication Abuse Detection Utility v".$version." (".$verdate.") \n".
         " Written by Ivo Truxa (c) 2014 <truxa\@truxoft.com> \n".
         " (based on the script detect_smtp_auth_abuse.pl by Todd Lyon (c) 2013) \n".
         "Modified by Markus Schmidt-Pauly June 2014 <msp\@schwarz.de> \n";
my $u2 = " usage: auth_checker.pl [options...] [logfile] \n".
         " Options: \n".
         " -e, --exclude <country> do not count logins from <country>\n".
         " -a, --access N Print accounts logged in more or equal N times \n".
         " -l, --limit N Print accounts accessed from N or more IP adresses \n".
         "     default 0; exceptions defined in the script not counted \n".
         "     unless limit is less than 0 (exceptions will be counted too) \n".
         " -i, --interval sets the interval for lookups in hours (default 24h) \n".
         " -q, --quiet shows no output when no matching IP address found \n".
         " -m, --mode <name> set to exim, dovecot, ssh, postfix, or apache mode (default exim) \n".
         ...
         " '/var/log/auth' for mode ssh, \n".
         " '/var/log/httpd/access.log' for mode apache and \n".
         " '/var/log/mail.info' for mode postfix \n";
my $changelog =
" 2.04 [Markus Schmidt-Pauly] 06/02/2014 \n".
" - added support for postfix sasl \n".
" - added options --exclude, --access and option --interval \n".
" - added sum logins/user \n".
" - fixed some problem with the -l option \n".
" - tuning up: filters already set by collecting the data \n".
" - additional entry in the data-array 'count' (used for --access)\n".
...
" the script will run even without the module, just with limited functionality \n".
...
# -----------------------------------------------------------------------
# creating a DateTime object for now:
# -----------------------------------------------------------------------

# for converting month to a number
my %mon2num = qw(
...
my $now = localtime();
my @tempdate = split(' ', $now);
my @temptime = split(':', $tempdate[3]);
my $year = $tempdate[4];
my $dt = DateTime->new (
                          year    =>  $year,
                          month   =>  $mon2num{ lc substr($tempdate[1], 0, 3) },
                          day     =>  $tempdate[2],
                          hour    =>  $temptime[0],
                          minute  =>  $temptime[1],
                          second  =>  $temptime[2],
...
sub timecomp {
# -----------------------------------------------------------------------
  # I get $date and $time from main and already have $dt
  my @date = split(' ', shift());
  my @time = split(':', shift());
  my $dt2 = DateTime->new(
                                year    =>  $year,
                                month   =>  $mon2num{ lc substr($date[0], 0, 3) },
                                day     =>  $date[1],
                                hour    =>  $time[0],
                                minute  =>  $time[1],
                                second  =>  $time[2],
  ...
  return $dur->years * 365 * 24 + $dur->months * 30 * 7 * 24 + $dur->weeks * 7 * 24 + $dur->days() * 24 + $dur->hours();
}
...
# -----------------------------------------------------------------------
my %opts;
GetOptions( \%opts, 'debug', 'limit:i', 'nopass', 'mode:s', 'user:s', 'geoip:i', 'version', 'help', 'changelog', 'quiet', 'nodate', 'nouser', 'exclude:s', 'interval:i', 'access:i');
...
$opts{'mode'} = ( defined $opts{'mode'} && $opts{'mode'}=~/all|exim|dovecot|ssh|openwm|apache|postfix/ )? $opts{'mode'} : 'exim';
# -----------------------------------------------------------------------
...
# POSTFIX: MAY 27 17:52:23 myhost postfix/smtpd[15562]: 74F155E4077: client=wick-asa5505.net.uta.at[62.218.39.138], sasl_method=CRAM-MD5, sasl_username=john.doe@anywhere.com

my @modes = ('exim','dovecot','ssh','openwm','postfix');
my %log_vars = (
...
my %log_pattern = (
        ...
        'postfix' => '^([a-zA-Z]{3} \d{2}) (\d{2}:\d{2}:\d{2}).*.*\[(\d+\.\d+\.\d+\.\d+)\].*sasl_username=([A-Za-z.-]*@[A-Za-z.-]*\.[A-Za-z]{2,6})'
);
if ( !($opts{'mode'} =~ /all/) ) {
    @modes = ($opts{'mode'});
}
    ...
    while (<$fh>) {
      ...
      if ( $_ =~ /$log_pattern{$mode}/ ) {
        ...
        if (defined $opts{'interval'}) {
          next if(timecomp($date, $time) > $opts{'interval'})
        }
        if ( defined $opts{'user'} ) { next if ($user ne $opts{'user'}); }
        ...
        if (defined $opts{'exclude'}) { next if (lc $country eq lc $opts{'exclude'}); }
...
# }
        ...
        if (defined $opts{'limit'}) { next if (scalar keys %{$f->{$user}->{'ip'}} < $opts{'limit'}); }
        if (defined $opts{'access'}) { next if ($f->{$user}->{'count'} < $opts{'access'}); }
        ...
        print "Count over all IP's: " . $f->{$user}->{'count'} . "\n";
        next if ($opts{'debug'});
...
if (defined $opts{'access'}) { print "\nLogin count overall: $logincount"; }
